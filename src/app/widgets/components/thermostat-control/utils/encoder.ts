...
function encodeDownlink(input) {
  ...
  function milesightDeviceEncode(payload) {
    ...
    if ("collection_interval" in payload) {
      encoded = encoded.concat(
        setCollectionInterval(payload.collection_interval)
      );
    }
    ...
    if ("dst_config" in payload) {
      encoded = encoded.concat(
        setDaylightSavingTime(
          payload.dst_config.enable,
          payload.dst_config.offset,
          payload.dst_config.start_time,
          payload.dst_config.end_time
        )
      );
    }
      ...
      if ("temperature_unit" in payload) {
        encoded = encoded.concat(
          setTemperatureControl(
            payload.temperature_control_mode,
            payload.temperature_target,
        ...
        encoded = encoded.concat(
          setTemperatureControlMode(payload.temperature_control_mode)
        );
      }
    }
    if ("temperature_control_enable" in payload) {
      encoded = encoded.concat(
        setTemperatureControlEnable(payload.temperature_control_enable)
      );
    }
    if ("temperature_calibration" in payload) {
      encoded = encoded.concat(
        setTemperatureCalibration(
          payload.temperature_calibration.enable,
          payload.temperature_calibration.temperature
        )
      );
    }
    if ("humidity_calibration" in payload) {
      encoded = encoded.concat(
        setHumidityCalibration(
          payload.humidity_calibration.enable,
          payload.humidity_calibration.humidity
        )
      );
    }
    if ("temperature_tolerance" in payload) {
      encoded = encoded.concat(
        setTemperatureTolerance(
          payload.temperature_tolerance.temperature_error,
          payload.temperature_tolerance.auto_control_temperature_error
        )
      );
    }
    if ("temperature_level_up_condition" in payload) {
      encoded = encoded.concat(
        setTemperatureLevelUpCondition(
          payload.temperature_level_up_condition.type,
          payload.temperature_level_up_condition.time,
          payload.temperature_level_up_condition.temperature_error
        )
      );
    }
    if ("outside_temperature_control_config" in payload) {
      encoded = encoded.concat(
        setOutsideTemperatureControl(
          payload.outside_temperature_control_config.enable,
          payload.outside_temperature_control_config.timeout
        )
      );
    }
    if ("outside_temperature" in payload) {
      encoded = encoded.concat(
        setOutsideTemperature(payload.outside_temperature)
      );
    }
    if ("freeze_protection_config" in payload) {
      encoded = encoded.concat(
        setFreezeProtection(
          payload.freeze_protection_config.enable,
          payload.freeze_protection_config.temperature
        )
      );
    }
    ...
    if ("fan_delay_enable" in payload) {
      encoded = encoded.concat(
        setFanModeWithDelay(payload.fan_delay_enable, payload.fan_delay_time)
      );
    }
    ...
    if ("humidity_range" in payload) {
      encoded = encoded.concat(
        setHumidityRange(payload.humidity_range.min, payload.humidity_range.max)
      );
    }
    if ("temperature_dehumidify" in payload) {
      encoded = encoded.concat(
        setTemperatureDehumidify(
          payload.temperature_dehumidify.enable,
          payload.temperature_dehumidify.temperature_tolerance
        )
      );
    }
    if ("fan_dehumidify" in payload) {
      encoded = encoded.concat(
        setFanDehumidify(
          payload.fan_dehumidify.enable,
          payload.fan_dehumidify.execute_time
        )
      );
    }
    ...
    if ("plan_schedule" in payload) {
      for (var i = 0; i < payload.plan_schedule.length; i++) {
        encoded = encoded.concat(
          setPlanSchedule(
            payload.plan_schedule[i].type,
            payload.plan_schedule[i].id,
            payload.plan_schedule[i].enable,
            payload.plan_schedule[i].week_recycle,
            payload.plan_schedule[i].time
          )
        );
      }
    }
    if ("plan_config" in payload) {
      encoded = encoded.concat(
        setPlanConfig(
          payload.plan_config.type,
          payload.plan_config.temperature_control_mode,
          payload.plan_config.fan_mode,
          payload.plan_config.temperature_target,
          payload.plan_config.temperature_error,
          payload.temperature_unit
        )
      );
    }
    if ("card_config" in payload) {
      encoded = encoded.concat(
        setCardConfig(
          payload.card_config.enable,
          payload.card_config.action_type,
          payload.card_config.in_plan_type,
          payload.card_config.out_plan_type,
          payload.card_config.invert
        )
      );
    }
    ...
    if ("multicast_group_config" in payload) {
      encoded = encoded.concat(
        setMulticastGroupConfig(payload.multicast_group_config)
      );
    }
    if ("d2d_master_enable" in payload || "d2d_slave_enable" in payload) {
      encoded = encoded.concat(
        setD2DEnable(payload.d2d_master_enable, payload.d2d_slave_enable)
      );
    }
    ...
    if ("control_permissions" in payload) {
      encoded = encoded.concat(
        setControlPermissions(payload.control_permissions)
      );
    }
  
    if ("offline_control_mode" in payload) {
      encoded = encoded.concat(
        setOfflineControlMode(payload.offline_control_mode)
      );
    }
  ...
  function reportStatus(report_status) {
    ...
    if (report_status_values.indexOf(report_status) === -1) {
      throw new Error(
        "report_status must be one of " + report_status_values.join(", ")
      );
    }
  ...
  function setDaylightSavingTime(enable, offset, start_time, end_time) {
    ...
    if (dst_config_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "dst_config.enable must be one of " + dst_config_enable_values.join(", ")
      );
    }
  ...
  function setTemperatureControlEnable(enable) {
    ...
    if (temperature_control_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "temperature_control.enable must be one of " +
          temperature_control_enable_values.join(", ")
      );
    }
  ...
  function setTemperatureControl(mode, temperature_target, temperature_unit) {
    ...
    if (temperature_control_mode_values.indexOf(mode) === -1) {
      throw new Error(
        "mode must be one of " + temperature_control_mode_values.join(", ")
      );
    }
    ...
    if (temperature_unit_values.indexOf(temperature_unit) === -1) {
      throw new Error(
        "temperature_unit must be one of " + temperature_unit_values.join(", ")
      );
    }
  
    let t =temperature_unit === 1
        ? temperature_target | 0x80
        : temperature_target & 0x7f;
  ...
  function setTemperatureCalibration(enable, temperature) {
    ...
    if (temperature_calibrate_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "temperature_calibration.enable must be one of " +
          temperature_calibrate_enable_values.join(", ")
      );
    }
  ...
  function setHumidityCalibration(enable, humidity) {
    ...
    if (humidity_calibrate_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "humidity_calibration.enable must be one of " +
          humidity_calibrate_enable_values.join(", ")
      );
    }
   ...
   */
  function setTemperatureTolerance(
    temperature_error,
    auto_control_temperature_error
  ) {
    ...
    if (typeof auto_control_temperature_error !== "number") {
      throw new Error(
        "temperature_tolerance.auto_control_temperature_error must be a number"
      );
    }
  ...
  function setTemperatureLevelUpCondition(type, time, temperature_error) {
    ...
    if (temperature_level_up_condition_type_values.indexOf(type) === -1) {
      throw new Error(
        "temperature_level_up_condition.type must be one of " +
          temperature_level_up_condition_type_values.join(", ")
      );
    }
    ...
    if (typeof temperature_error !== "number") {
      throw new Error(
        "temperature_level_up_condition.temperature_error must be a number"
      );
    }
  ...
  function setTemperatureTarget(temperature_control_mode, temperature_target) {
    ...
    if (temperature_mode_values.indexOf(temperature_control_mode) === -1) {
      throw new Error(
        "temperature_control_mode must be one of " +
          temperature_mode_values.join(", ")
      );
    }
    if (typeof temperature_target !== "number") {
      throw new Error("temperature_target must be a number");
  ...
  function setOutsideTemperatureControl(enable, timeout) {
    ...
    if (outside_temperature_control_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "outside_temperature_control_config.enable must be one of " +
          outside_temperature_control_enable_values.join(", ")
      );
    }
    if (enable && typeof timeout !== "number") {
      throw new Error(
        "outside_temperature_control_config.timeout must be a number"
      );
    }
  ...
  function setTemperatureDehumidify(enable, temperature_tolerance) {
    ...
    if (dehumidify_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "dehumidify.enable must be one of " + dehumidify_enable_values.join(", ")
      );
    }
    if (enable) {
      // default value
  ...
  function setFanDehumidify(enable, execute_time) {
    ...
    if (fan_dehumidify_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "fan_dehumidify.enable must be one of " +
          fan_dehumidify_enable_values.join(", ")
      );
    }
  ...
  function setFreezeProtection(enable, temperature) {
    ...
    if (freeze_protection_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "freeze_protection_config.enable must be one of " +
          freeze_protection_enable_values.join(", ")
      );
    }
    if (enable && typeof temperature !== "number") {
  ...
  function setFanModeWithDelay(fan_delay_enable, fan_delay_time) {
    ...
    if (fan_delay_enable_values.indexOf(fan_delay_enable) === -1) {
      throw new Error(
        "fan_delay_enable must be one of " + fan_delay_enable_values.join(", ")
      );
    }
  ...
  function setPlanSchedule(type, id, enable, week_recycle, time) {
    ...
    if (plan_schedule_type_values.indexOf(type) === -1) {
      throw new Error(
        "plan_schedule[].type must be one of " +
          plan_schedule_type_values.join(", ")
      );
    }
    if (typeof id !== "number") {
    ...
    if (plan_schedule_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "plan_schedule[].enable must be one of " +
          plan_schedule_enable_values.join(", ")
      );
    }
    ...
    for (var i = 0; i < week_recycle.length; i++) {
      ...
      if (plan_schedule_week_recycle_values.indexOf(day) === -1) {
        throw new Error(
          "week_recycle must be one of " +
            plan_schedule_week_recycle_values.join(", ")
        );
      }
     let offset = plan_schedule_week_recycle_values.indexOf(day);
      days |= 1 << offset;
    }
  ...
  function setPlanConfig(
    ...
    if (plan_config_type_values.indexOf(type) === -1) {
      throw new Error(
        "plan_config.type must be one of " + plan_config_type_values.join(", ")
      );
    }
    if (
      plan_config_temperature_control_mode_values.indexOf(
        temperature_control_mode
      ) === -1
    ) {
      throw new Error(
        "plan_config.temperature_control_mode must be one of " +
          plan_config_temperature_control_mode_values.join(", ")
      );
    }
    if (plan_config_fan_mode_values.indexOf(fan_mode) === -1) {
      throw new Error(
        "plan_config.fan_mode must be one of " +
          plan_config_fan_mode_values.join(", ")
      );
    }
    if (typeof temperature_target !== "number") {
      throw new Error("temperature_target must be a number");
  ...
  function setCardConfig(
    ...
    if (card_config_enable_values.indexOf(enable) === -1) {
      throw new Error(
        "card_config.enable must be one of " +
          card_config_enable_values.join(", ")
      );
    }
    var card_config_action_type_values = [0, 1];
    if (enable && card_config_action_type_values.indexOf(action_type) === -1) {
      throw new Error(
        "card_config.action_type must be one of " +
          card_config_action_type_values.join(", ")
      );
    }
    ...
    if (action_type === 1) {
      ...
      if (card_config_plan_type_values.indexOf(in_plan_type) === -1) {
        throw new Error(
          "card_config.in_plan_type must be one of " +
            card_config_plan_type_values.join(", ")
        );
      } else {
      ...
      if (card_config_plan_type_values.indexOf(out_plan_type) === -1) {
        throw new Error(
          "card_config.out_plan_type must be one of " +
            card_config_plan_type_values.join(", ")
        );
      } else {
    ...
    if (enable && card_config_invert_values.indexOf(invert) === -1) {
      throw new Error(
        "card_config.invert must be one of " +
          card_config_invert_values.join(", ")
      );
    }
    ...
    buffer.writeUInt8(enable);
    buffer.writeUInt8(
      enable ? card_config_action_type_values.indexOf(action_type) : 0
    );
  ...
  function setD2DEnable(d2d_master_enable, d2d_slave_enable) {
    ...
    if (d2d_master_enable !== undefined) {
      if (d2d_enable_values.indexOf(d2d_master_enable) === -1) {
        throw new Error(
          "d2d_master_enable must be one of " + d2d_enable_values.join(", ")
        );
      }
      mask |= 1 << 0;
    ...
    if (d2d_slave_enable !== undefined) {
      if (d2d_enable_values.indexOf(d2d_slave_enable) === -1) {
        throw new Error(
          "d2d_slave_enable must be one of " + d2d_enable_values.join(", ")
        );
      }
      mask |= 1 << 1;
  ...
  function setMulticastGroupConfig(multicast_group_config) {
    ...
    if ("group1_enable" in multicast_group_config) {
      if (
        group_enable_values.indexOf(multicast_group_config.group1_enable) === -1
      ) {
        throw new Error(
          "group1_enable must be one of " + group_enable_values.join(", ")
        );
      }
      mask_id |= 1 << 0;
    ...
    if ("group2_enable" in multicast_group_config) {
      if (
        group_enable_values.indexOf(multicast_group_config.group2_enable) === -1
      ) {
        throw new Error(
          "group2_enable must be one of " + group_enable_values.join(", ")
        );
      }
      mask_id |= 1 << 1;
    ...
    if ("group3_enable" in multicast_group_config) {
      if (
        group_enable_values.indexOf(multicast_group_config.group3_enable) === -1
      ) {
        throw new Error(
          "group3_enable must be one of " + group_enable_values.join(", ")
        );
      }
      mask_id |= 1 << 2;
    ...
    if ("group4_enable" in multicast_group_config) {
      if (
        group_enable_values.indexOf(multicast_group_config.group4_enable) === -1
      ) {
        throw new Error(
          "group4_enable must be one of " + group_enable_values.join(", ")
        );
      }
      mask_id |= 1 << 3;
  ...
  function setD2DMasterConfig(
    ...
    if (mode_values.indexOf(mode) === -1) {
      throw new Error(
        "d2d_master_config.mode must be one of " + mode_values.join(", ")
      );
    }
    var enable_values = [0, 1];
    if (enable_values.indexOf(enable) === -1) {
      throw new Error(
        "d2d_master_config.enable must be one of " + enable_values.join(", ")
      );
    }
    if (enable && enable_values.indexOf(uplink_enable) === -1) {
      throw new Error(
        "d2d_master_config.uplink_enable must be one of " +
          enable_values.join(", ")
      );
    }
    if (enable && enable_values.indexOf(time_enable) === -1) {
      throw new Error(
        "d2d_master_config.time_enable must be one of " + enable_values.join(", ")
      );
    }
  ...
  function setD2DSlaveConfig(id, enable, d2d_cmd, action_type, action) {
    ...
    if (enable_values.indexOf(enable) === -1) {
      throw new Error(
        "d2d_slave_config.enable must be one of " + enable_values.join(", ")
      );
    }
    var action_type_values = [0, 1];
    if (action_type_values.indexOf(action_type) === -1) {
      throw new Error(
        "d2d_slave_config.action_type must be one of " +
    ...
    if (action_type === 0) {
      ...
      if (action_values.indexOf(action) === -1) {
        throw new Error(
          "d2d_slave_config.action must be one of " + action_values.join(", ")
        );
      }
    }
    if (action_type === 1) {
  ...
  function setControlPermissions(control_permissions) {
    ...
    if (control_permission_values.indexOf(control_permissions) === -1) {
      throw new Error(
        "control_permissions must be one of " +
          control_permission_values.join(", ")
      );
    }
  ...
  function setOfflineControlMode(offline_control_mode) {
    ...
    if (offline_control_mode_values.indexOf(offline_control_mode) === -1) {
      throw new Error(
        "offline_control_mode must be one of " +
          offline_control_mode_values.join(", ")
      );
    }
   ...
   */
  function setTemperatureAlarmConfig(
    alarm_type,
    condition,
    min,
    max,
    ...
    if (alarm_type_values.indexOf(alarm_type) === -1) {
      throw new Error(
        "alarm_type must be one of " + alarm_type_values.join(", ")
      );
    }
  ...
  export function sendCommand(appName:string,token:string, deviceId:string,json:{}){
      let data = {
          ...
          hex: bytesToHex(encodeDownlink({
              data:json
            }).bytes),
            base64: hexToBase64(bytesToHex(encodeDownlink({
              data:json
            }).bytes))
